    ...
    def generate_network_diagram(self, topology_data: Dict[str, Any], title: str = "Network Topology") -> str:
        ...
        try:
            ...
            for device in devices:
                vendor = (device.get("vendor") or "generic").lower()
...
"""
            ...
            for device in devices:
                ...
                if is_replaced:
                    label_parts = [f" {icon} {self._sanitize_id(device.get('id', 'unknown'))}"]
                    if model:
...
"""
            ...
            for device in original_devices:
                ...
                device_type = device.get("type", "generic")
                vendor = (device.get("vendor") or "generic")
                model = device.get("model", "")
...
"""
            ...
            for device in modified_devices:
                ...
                if is_replaced:
                    ...
                    if model:
                        label = f" {icon} {vendor.upper()}<br/>{model}<br/>UPGRADED"
                    else:
                        label = f" {icon} {vendor.upper()}<br/>{device_type.title()}<br/>UPGRADED"
                else:
...
"""
    ...
    def _generate_dynamic_fallback_connections(self, devices: List[Dict[str, Any]]) -> str:
        ...
        for device in devices:
            ...
            # Check for management devices first
            if device_type == 'management':
                device_categories['management'].append(device)
            # Check for core devices
            elif any(indicator in device_id or indicator in device_model for indicator in core_indicators):
        ...
        for edge in device_categories['edge']:
            ...
            connections += f"    {edge_id} ---|Internet| internet\n"
        
        # Connect management devices to core and distribution
        for mgmt in device_categories['management']:
            mgmt_id = self._sanitize_id(mgmt.get("id"))
            for core in device_categories['core']:
                core_id = self._sanitize_id(core.get("id"))
                connections += f"    {mgmt_id} -.->|Manages| {core_id}\n"
            for dist in device_categories['distribution']:
                dist_id = self._sanitize_id(dist.get("id"))
                connections += f"    {mgmt_id} -.->|Manages| {dist_id}\n"
