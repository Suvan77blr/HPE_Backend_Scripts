...
class NetworkIntegrationAgent:
    def __init__(self, vector_store: VectorStore, llm_service: LLMService, web_searcher: Optional[WebSearcher] = None):
        ...
        self.orchestrator = GroqOrchestrator(vector_store, llm_service, web_searcher)
    ...
    def _is_tool_query(self, analysis):
        # Check if analysis indicates a tool-based query
        return analysis.get('intent', '') == 'tool'
    ...
    def generate_response(self, query: str, topology_context: Optional[Dict[str, Any]] = None) -> str:
        ...
        try:
            ...
            # Use orchestrator for tool-based queries
            if self._is_tool_query(analysis):
                # tool-based response logic is here. => by Groq orchestrator.
                plan = self.orchestrator.route_query(query)
                return self.orchestrator.execute_workflow(plan)
